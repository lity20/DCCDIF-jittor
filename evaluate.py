...
def evaluate(config):
    ...
    mean_cd, mean_nc, mean_fs = np.mean(cd_list), np.mean(nc_list), np.mean(fs_list)
    print('Chamfer-L2: %f, Normal Consistency: %f, F-Score: %f.' % (mean_cd, mean_nc, mean_fs))

    classes = [l.split('/')[0] for l in shape_ids]
    shape_ids = [l.split('/')[1] for l in shape_ids]
    data_frame = pandas.DataFrame({'class': classes + [''],
                                   'shape_id': shape_ids + ['mean'],
                                   'cd': cd_list.tolist() + [mean_cd],
                                   'nc': nc_list.tolist() + [mean_nc],
                                   'fs': fs_list.tolist() + [mean_fs]})
